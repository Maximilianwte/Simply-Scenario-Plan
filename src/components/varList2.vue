<template>
  <div :id="componentId" class="draggableList text-2xl min-h-screen w-80 pt-6 border-l-4 border-r-4 border-dashed">
            ...
            <div id="top" class="flex-col w-full py-1 px-2 justify-around">
              <div id="title" class="">
                <form>
                  ...
                  <input type="text" v-model="item.title" @change="changeVar(item.id, 'title')"
            ...
            <div id="below" class="flex-col w-full py-1 px-2 justify-around">
              <div class="w-full" id="probability">
                <form>
                  ...
                  <input type="number" v-model="item.prob" @change="changeVar(item.id, 'prob')"
                    @focus="cacheValues(item.id, 'prob')" min="0" max="100" step="any"
                    class="cursor-pointer text-dark w-16" ondblclick="this.setSelectionRange(0, this.value.length)" />
                  <label class="ml-1">%</label>
                </form>
              </div>
            </div>
          </div>
          <div v-if="IDsetImpact == item.id || item.impact[0] == ''" id="changeVarFloating" class="absolute flex">
            <div id="arrow" class="flex-col">
              <svg class="w-10" style="transform: rotateZ(270deg)" xmlns="http://www.w3.org/2000/svg"
            ...
            <div id="outcomeVarMenu" class="bg-gray-100 px-2 py-4 rounded text-base">
              ...
              <div v-for="outcomevar in getOutcomeVars" :key="outcomevar.displayId" :title="outcomevar.title"
                class="variableItem py-1 px-3 rounded-full mt-1 flex">
                <div class="w-10 h-10 rounded-full flex-col"
                  :style="{ backgroundColor: getColorMode(outcomevar.color) }">
                  <h6>{{ outcomevar.title[0] }}</h6>
                </div>
                <input type="number" v-model="item.impact[outcomevar.id]"
                  @focus="cacheValues(item.id, 'impact', outcomevar.id)"
                  @change="changeVar(item.id, 'impact', outcomevar.id)" step="any"
                  class="cursor-pointer text-dark w-16 ml-2 border-2 border-gray-200 text-right"
                  ondblclick="this.setSelectionRange(0, this.value.length)" />
                <form>
                  <select v-model="item.unit[outcomevar.id]" name="unit"
