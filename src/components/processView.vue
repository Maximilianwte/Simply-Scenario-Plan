<template>
  <div :id="'processView_' + outcomeVar.id">
    <svg v-for="id in nConnectionsOutput" :key="'svg_' + outcomeVar.id + '_' + id"
      :id="'svg_' + outcomeVar.id + '_' + id" class="absolute clickThrough" width="0" height="0">
      <path :id="'path' + id" d="M0 0" stroke-width="0.21rem" style="stroke: #cbd5e0; fill: none" />
    </svg>
    <div class="row inline-block flex text-dark">
      <div v-for="(list, list_index) in data" :key="'list ' + list_index" class="flex-col mr-16">
        <div v-for="(item, index) in list" :key="index"
          :id="'outputVar_' + outcomeVar.id + '_' + (list_index + 1) + '_' + item.id"
          class="scenarioVar px-4 py-6 w-48 text-center mt-2 text-xl rounded" :style="getColor(item.color)">
          {{ item.title }}
      ...
      <div class="outputVarContainer ml-20 flex-col">
        <div :id="'outcomeVar_' + outcomeVar.id" class="scenarioVar px-4 py-6 w-32 text-center mt-2 text-xl rounded"
          :style="getColor(outcomeVar.color)">
          {{outcomeVar.title}}
        </div>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      connectShapes() {
        ...
        store.commit("addAllOutputConnections", {
          id: this.outcomeVar.id,
